...
#ifndef lint
static const char sccsid[] = "@(#)java_util.cpp	10.1 (Sleepycat) 11/10/97";
#endif /* not lint */
    ...
    , java_array_len_(0)
{
    ...
    jclass dbClass = get_class(jnienv, name_DBT);

    if (dbt && dbt->array_ != 0) {
        if (!dbt->dbowned_) {
            dbt->data = jnienv->GetByteArrayElements(dbt->array_, NULL);
            dbt->data = ((char*)dbt->data) + dbt->offset_;
            int len = jnienv->GetArrayLength(dbt->array_);
            java_array_len_ = len;
            if ((unsigned int)len < dbt->ulen)
                dbt->ulen = len - dbt->offset_;
        }
    }
    else if (dbt) {
        dbt->data = 0;
...
LockedDBT::~LockedDBT()
{
    if (dbt && dbt->array_) {
        if (!dbt->dbowned_) {
            dbt->data = ((char*)dbt->data) - dbt->offset_;
            env_->ReleaseByteArrayElements(dbt->array_, (jbyte *)dbt->data, 0);
            dbt->data = 0;
        }
        else {
            int len = env_->GetArrayLength(dbt->array_);
            if (dbt->size > (unsigned int)len) {
